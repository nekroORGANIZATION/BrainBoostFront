...
export default function TipOfDayCard({ className = '' }: { className?: string }) {
  ...
  async function load() {
    ...
    try {
      ...
      const data = (await r.json()) as Tip;
      setTip(data);
    } catch (e: any) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 flex items-center gap-2">
        ...
        {tip?.source && <span className="text-xs text-slate-500">Джерело: {tip.source}</span>}
      </div>
    </motion.div>
  );
}
