...
/* ---------- Reusable UI ---------- */
type CardProps = { title: string; description: string; width?: string };
...
function CourseCard({ title, description, width = 'auto' }: CardProps) {
  ...
  return (
    <motion.div variants={fadeUp} className="flex flex-col md:flex-row items-start gap-6 w-full">
      ...
      <div className="p-4 sm:p-6 w-full max-w-[800px] text-black">
        <p className="font-[Mulish] font-medium text-base sm:text-lg md:text-xl leading-relaxed break-words">
          {description}
        </p>
      </div>
    </motion.div>
...
function WhoIsThisCourseFor() {
  const cards: CardProps[] = [
    { title: 'Усім, хто хоче опанувати нову професію', description: 'Ви у пошуках себе... нових можливостей і нових цілей.', width: '641px' },
    { title: 'Керівникам та власникам бізнесу', description: 'Шукаєте ефективні курси... та масштабуванню бізнесу.', width: '535px' },
    { title: 'Підприємцям', description: 'Комплексні знання з інтернет-маркетингу... які конкретні результати має приносити команда.', width: '334px' },
    { title: 'Маркетологам', description: 'Стати експертом у сфері інтернет-просування... та зростати у доходах.', width: '334px' },
  ];
  ...
  return (
    <section className="relative w-full backdrop-blur-sm pt-16 sm:pt-24 lg:pt-32 px-4 sm:px-6 lg:px-8 pb-16 sm:pb-24 lg:pb-32 rounded-xl shadow-lg">
      <motion.h2 variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} className="font-[Afacad] font-bold text-2xl sm:text-3xl lg:text-[42px] text-center mb-10 sm:mb-16" style={{ maxWidth: 389, margin: '0 auto' }}>
      ...
      <motion.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={{ once: true }} className="flex flex-col gap-8 sm:gap-12">
        {cards.map((c, i) => <CourseCard key={i} {...c} />)}
      </motion.div>
    </section>
  );
}
        ...
        {mentors.map((m, i) => (
          <div key={i} className="w-[298px] h-[396px] relative flex-shrink-0 rounded-md overflow-hidden">
            ...
            <div className="absolute bottom-4 left-4 right-4 text-center text-white p-2 rounded bg-black/50">
              <h3 className="font-[Mulish] font-bold text-[20px]">{m.name}</h3>
              <p className="font-[Mulish] text-[14px]">{m.role}</p>
            </div>
          </div>
        ))}
      </motion.div>
  ...
  return (
    <section className="w-full max-w-[1200px] mx-auto flex flex-col md:flex-row gap-6 sm:gap-8 mt-12 sm:mt-20 md:mt-40 px-4 sm:px-6">
      <ol className="w-full md:w-[494px] flex flex-col gap-4 sm:gap-6 md:gap-10 font-bold text-base sm:text-[20px] text-black list-decimal list-inside">
        {topics.map((t, i) => (<li key={i} className="leading-snug sm:leading-tight">{t}</li>))}
      </ol>
...
function CoursePreview() {
  const items = [
    { title: 'Практичні домашні завдання', text: 'Ви одразу відточуєте отримані знання на практиці...' },
    { title: 'Викладачі-практики', text: 'Навчання у людей з багаторічним досвідом...' },
    { title: 'Навчання у зручний для вас час', text: 'Уроки на освітній платформі. Налаштуйте графік самі.' },
    { title: 'Диплом або сертифікат', text: 'Після навчання отримуєте документ, що підсилить працевлаштування.' },
    { title: 'Можливість вчитися з нуля', text: 'Корисно і для новачків, і для спеціалістів.' },
    { title: 'Ментор та координатори', text: 'Команда підтримки допомагає з усіх питань.' },
  ];
  ...
  return (
    <section className="w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-6 py-8 sm:py-12">
      ...
      <p className="mt-6 sm:mt-8 w-full sm:max-w-[656px] text-base sm:text-[20px] font-medium leading-relaxed text-black mx-auto sm:mx-0">
        Учасники працюють у групах під керівництвом викладача. Реальні завдання + зворотний зв’язок.
      </p>
    </section>
  );
}
  ...
  return (
    <section className="w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-6 py-8 sm:py-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-12 justify-items-center mb-8 sm:mb-12">
        ...
        <div className="flex flex-col items-center"><div className="w-36 h-36 sm:w-44 sm:h-44 flex items-center justify-center rounded-full bg-[#82A1FF99]"><Target className="w-20 h-20 sm:w-24 sm:h-24 text-[#0A2578]" /></div><p className="mt-2 sm:mt-4 text-base sm:text-lg font-bold text-black text-center">Аналіз ринку та ЦА</p></div>
      </div>
...
function LearningProcess() {
  const items = [
    { icon: <BookOpen className="w-[100px] h-[100px] text-[#0A2578]" />, title: 'Теорія + практика:', text: 'Відеоуроки → завдання з шаблонами.' },
    { icon: <CheckSquare className="w-[100px] h-[100px] text-[#0A2578]" />, title: 'Домашні завдання та тестування:', text: 'Перевірка робіт, відповіді, супровід куратора.' },
    { icon: <Users className="w-[100px] h-[100px] text-[#0A2578]" />, title: 'Живі Q&A-сесії:', text: 'Онлайн «Питання–Відповідь» з менторами.' },
    { icon: <Award className="w-[100px] h-[100px] text-[#0A2578]" />, title: 'Диплом:', text: 'Підсумкове тестування та офіційний диплом.' },
  ];
    ...
    <section className="w-full py-12 flex flex-col items-center">
      ...
      <div className="bg-white border border-black/30 rounded-2xl shadow-lg p-10 w-[1100px] max-w-full flex flex-col gap-10">
        ...
        <div className="rounded-xl border border-black/20 p-6">
          <div className="grid grid-cols-3 text-center font-medium font-[Mulish] text-lg text-black mb-3"><p>Міста</p><p>Зарплата</p><p>Зміна за рік</p></div>
...
function EmploymentHelp() {
  const steps = [
    ...
    { number: '2', text: (<><p>Кожен диплом оснащений <b>QR-кодом</b>: роботодавець може швидко</p><p>перевірити його справжність і статистику навчання.</p></>) },
    { number: '3', text: (<><p>Доступ до закритого курсу працевлаштування <b>“BrainBoost Talents”</b>:</p><p>резюме, співбесіди, працевлаштування в ІТ.</p></>) },
  ];
...
function BookingSection() {
  ...
  return (
    <section id="signup" className="booking-section flex flex-col items-center py-10 px-4 sm:px-6">
      ...
      <div className="relative mt-6 sm:mt-10 w-full max-w-[850px] rounded-[30px] bg-[#82A1FF] shadow-[9px_-9px_10px_rgba(0,0,0,0.25),-9px_9px_10px_rgba(0,0,0,0.25)] flex flex-col sm:flex-row gap-6 sm:gap-10 p-6 sm:p-16 justify-center">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 sm:gap-6 w-full sm:w-[400px] relative z-10" noValidate>
          ...
          {status && (
            <p className={`text-[14px] font-[Mulish] font-semibold px-2 sm:px-3 py-2 rounded-md ${status.type === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`} role="status" aria-live="polite">
              {status.message}
            </p>
          )}
        </form>
...
function FAQSection() {
  const faqs: FAQ[] = [
    { question: 'Чому digital-маркетологи такі популярні?', answer: 'Після 2020 року онлайн присутність стала критичною, попит зріс, якісних фахівців бракує.' },
    { question: 'Я зовсім новачок. Курс підходить мені?', answer: 'Так. Перші блоки — для старту з нуля; фахівці структурують знання.' },
    { question: 'Як довго буде доступний курс?', answer: 'Доступ на рік — достатньо, щоб пройти всі модулі.' },
    { question: 'Чи гарантовано отримаю сертифікат?', answer: '265 балів — диплом, менше — сертифікат про проходження.' },
  ];
  ...
  return (
    <section className="px-4 sm:px-6 py-10 sm:py-16">
      ...
      <div className="max-w-full sm:max-w-4xl mx-auto flex flex-col gap-3 sm:gap-4 mb-10 sm:mb-20">
        {faqs.map((faq, i) => (
          <div key={i} className="bg-white rounded-xl shadow-md p-4 sm:p-5 cursor-pointer transition hover:shadow-lg" onClick={() => setOpenIndex(openIndex === i ? null : i)}>
            <div className="flex justify-between items-center">
              <p className="text-base sm:text-lg font-bold">{faq.question}</p>
              <ChevronDown className={`w-5 h-5 sm:w-6 sm:h-6 transition-transform duration-300 ${openIndex === i ? 'rotate-180' : ''}`} />
            </div>
            {openIndex === i && <p className="mt-2 sm:mt-3 text-gray-700 text-sm sm:text-base">{faq.answer}</p>}
          </div>
        ))}
      </div>
