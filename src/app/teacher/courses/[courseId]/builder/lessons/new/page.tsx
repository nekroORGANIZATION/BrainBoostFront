...
const CALL = {
  info: (txt='Пояснення / контекст') =>
    `<div class="cxe-callout cxe-callout-info"><strong>Інфо:</strong> ${txt}</div>`,
  warn: (txt='Застереження / важливо') =>
    `<div class="cxe-callout cxe-callout-warn"><strong>Увага:</strong> ${txt}</div>`,
  ok: (txt='Порада / best practice') =>
    `<div class="cxe-callout cxe-callout-ok"><strong>Порада:</strong> ${txt}</div>`,
};

const COLS2 =
  `<div class="cxe-cols">
    <div><h3>Колонка 1</h3><p>Вміст…</p></div>
    <div><h3>Колонка 2</h3><p>Вміст…</p></div>
  </div><p><br/></p>`;
...
function prettyError(e: any): string {
  ...
  if (typeof data === 'string') {
    if (data.includes('UNIQUE constraint failed') || data.includes('IntegrityError')) {
      return 'У цьому курсі вже існує урок з такою назвою. Змініть назву або збережіть ще раз — я додам суфікс автоматично.';
    }
    return data;
  }
  ...
  try {
    ...
    return status ? `HTTP ${status}` : 'Сталася помилка.';
  }
}
function isUniqueTitleError(e:any){
  const s = e?.response?.data;
  return typeof s === 'string' && s.includes('UNIQUE constraint failed') && s.includes('lesson_lesson.course_id, lesson_lesson.title');
}
async function suggestUniqueTitle(base: string, courseId: number) {
  const clean = (base || 'Новий урок').trim();
  try {
    const r = await http.get('/lesson/admin/lessons/', { params: { course: courseId, ordering: 'id' } });
    const arr = asArray<any>(r.data);
    const set = new Set(arr.map(x => String(x?.title || '').toLowerCase()));
...
export default function LessonEditorPage() {
  ...
  const paramLessonId = Number(Array.isArray(params.lessonId)?params.lessonId[0]:params.lessonId);
  const hasParamLessonId = !!paramLessonId && !Number.isNaN(paramLessonId);

  // модуль, який прийшов з URL як початковий хінт (не обов'язково)
  const moduleFromUrl = Number(search.get('module') || '') || null;

  const { accessToken } = useAuth();

  // meta
  const [lessonTitle, setLessonTitle] = useState('Новий урок');
  const [lastSaved, setLastSaved] = useState<string | null>(null);
  const [err, setErr] = useState<string|null>(null);

  // модулі для селекту
  const [modules, setModules] = useState<Module[]>([]);
  const [moduleIdState, setModuleIdState] = useState<number|null>(moduleFromUrl);
  ...
  const [sel, setSel] = useState({
    ...
    ul:false, ol:false, link:false, blockquote:false,
  });
  ...
  /* ---------- завантажити модулі курсу для селекту ---------- */
  useEffect(() => {
    if (!courseId) return;
    (async () => {
      try {
        const r = await http.get('/lesson/admin/modules/', { params: { course: courseId, ordering: 'order' } });
        const arr = asArray<Module>(r.data).sort((a,b)=>a.order-b.order||a.id-b.id);
        setModules(arr);
        // якщо немає ще стейту — встановимо перший видимий
        if (!hasParamLessonId && moduleIdState==null && arr.length) {
          setModuleIdState(arr[0].id);
        }
      } catch {}
    })();
  ...
  function insertHTML(htmlFrag: string, ref: React.RefObject<HTMLDivElement | null>) {
    ...
    if (!sel || sel.rangeCount === 0 || !el) {
      el?.insertAdjacentHTML('beforeend', htmlFrag);
      return;
    }
  ...
  const setAlign = (a: 'left'|'center'|'right'|'justify') =>
    exec('justify' + a[0].toUpperCase()+a.slice(1));
  ...
  const addLink = () => {
    const url = prompt('Вставити URL посилання:');
    if (!url) return;
    exec('createLink', url);
  };
  ...
  const insertImage = async () => {
    ...
    if (fromFile) {
      ...
      inp.onchange = async () => {
        const f = inp.files?.[0];
        if (!f) return;
        const url = await uploadImageSmart(f);
        insertHTML(`<figure><img src="${url}" alt="" /><figcaption>Підпис (необовʼязково)</figcaption></figure><p><br/></p>`, editorRef);
      };
      inp.click();
    } else {
      const url = prompt('Вставити URL зображення:');
      if (url) insertHTML(`<figure><img src="${url}" alt="" /><figcaption>Підпис (необовʼязково)</figcaption></figure><p><br/></p>`, editorRef);
    }
  };

  const insertYouTube = () => {
    const url = prompt('Вставити YouTube URL:');
    if (!url) return;
    const yt = /youtu\.be|youtube\.com/.test(url);
    let embed = url;
    if (yt) {
      try { const u = new URL(url); const v = u.searchParams.get('v') || url.split('/').pop() || ''; embed = `https://www.youtube.com/embed/${v}`; } catch {}
    }
    insertHTML(`<p><iframe class="cxe-video" src="${embed}" allowfullscreen></iframe></p>`, editorRef);
  };

  const insertTable = () => {
    const r = Number(prompt('К-сть рядків:', '3') || '3');
    const c = Number(prompt('К-сть колонок:', '3') || '3');
    insertHTML(tableHTML(Math.max(1, r), Math.max(1, c), true), editorRef);
  };

  const insertChecklist = () => {
    insertHTML(`<ul class="cxe-checklist"><li data-checked="false">Завдання 1</li><li data-checked="false">Завдання 2</li><li data-checked="false">Завдання 3</li></ul><p><br/></p>`, editorRef);
  };

  const toggleBlockquote = () => {
    const bq = closest(editorRef.current, 'blockquote');
    if (bq) unwrap(bq as HTMLElement);
  ...
  const saveNow = useCallback(async (goToProgram=false) => {
    ...
    try {
      ...
      let id = hasParamLessonId ? paramLessonId : null;

      if (!id) {
        // підібрати унікальну назву
        let titleToUse = await suggestUniqueTitle((lessonTitle || 'Новий урок').trim(), courseId);

        try {
          const res = await http.post(`/lesson/admin/lessons/`, {
            course: courseId,
            module_id: moduleIdState,   // ← ГОЛОВНЕ: завжди шлемо module_id
            title: titleToUse,
            status: 'draft',
            summary: summaryText,
            contents: blocks,
          ...
          id = res.data?.id;
          if (!id) throw new Error('Сервер не повернув id уроку.');
        } catch (e:any) {
          if (isUniqueTitleError(e)) {
            titleToUse = await suggestUniqueTitle((lessonTitle || 'Новий урок').trim(), courseId);
            const res2 = await http.post(`/lesson/admin/lessons/`, {
              course: courseId,
              module_id: moduleIdState,
              title: titleToUse,
              status: 'draft',
              summary: summaryText,
              contents: blocks,
        ...
        // лишаємось на редакторі, але вже як edit
        router.replace(`/teacher/courses/${courseId}/builder/lessons/${id}/edit`);
        if (goToProgram) router.push(`/teacher/courses/${courseId}/builder/program`);
        return;
      }

      // UPDATE
      await http.patch(`/lesson/admin/lessons/${id}/`, {
        title: (lessonTitle || '').trim(),
        summary: summaryText,
        contents: blocks,
        module_id: moduleIdState,      // ← закріплюємо/оновлюємо модуль
      });
  ...
  }, [paramLessonId, hasParamLessonId, lessonTitle, sourceMode, html, courseId, moduleIdState, router]);

  /* ---------- load title & content (read blocks) ---------- */
  useEffect(() => {
    if (!hasParamLessonId) return;
    let cancel = false;
    (async () => {
      try {
        const l = await http.get(`/lesson/admin/lessons/${paramLessonId}/`);
        if (cancel) return;
        const data = l.data || {};
        setLessonTitle(data?.title || 'Новий урок');

        // підхопимо модуль уроку (важливо для PATCH)
        setModuleIdState(data?.module?.id ?? moduleFromUrl ?? null);
        ...
        else if (data?.summary) initialHtml = String(data.summary);
    ...
    return () => { cancel = true; };
  }, [hasParamLessonId, paramLessonId]); // eslint-disable-line
  ...
  useEffect(() => {
    const onSel = () => {
      ...
      setSel({
        ...
        link: q('unlink') === false,
        blockquote: !!closest(editorRef.current, 'blockquote'),
        align,
      });
    };
  ...
  useEffect(() => {
    ...
    const onDrop = async (ev: DragEvent) => {
      ...
      if (file && file.type.startsWith('image/')) {
        const url = await uploadImageSmart(file);
        insertHTML(`<figure><img src="${url}" alt="" /><figcaption>Підпис</figcaption></figure><p><br/></p>`, editorRef);
      }
    };
  ...
  useEffect(() => {
    const calc = () => {
      ...
      setStats({ words, chars, read: `${mins} хв` });

      const out: Array<{id:string;text:string;level:number}> = [];
      if (root) {
        root.querySelectorAll('h1,h2,h3').forEach((h,i) => {
          const id = `h-${i}`;
          (h as HTMLElement).id = (h as HTMLElement).id || id;
          out.push({ id: (h as HTMLElement).id, text: (h.textContent||'').trim(), level: Number(h.nodeName[1]) });
        });
      }
      setOutline(out);
    };
    ...
    return (
      <main className="min-h-screen bg-[url('/images/back.png')] bg-cover bg-top grid place-items-center px-6">
        <div className="max-w-lg rounded-2xl bg-white/95 ring-1 ring-[#E5ECFF] p-6 text-center">
          ...
          <p className="text-slate-600 mt-1">Щоб створювати уроки — увійди.</p>
          <div className="mt-4">
            <Link href="/login" className="inline-flex items-center px-5 py-2 rounded-xl bg-[#1345DE] text-white font-semibold">Увійти</Link>
          </div>
        </div>
      </main>
    );
  ...
  return (
    <main className="min-h-screen bg-[url('/images/back.png')] bg-cover bg-top">
      <style>{`
        ...
        .cxe .cxe-checklist li::before {
          content: attr(data-checked);
          display: inline-block; width: 16px; height: 16px; border: 1px solid #94a3b8;
          position: absolute; left: 0; top: 2px; border-radius: 4px; background: #fff; color: transparent;
        }
        .cxe .cxe-checklist li[data-checked="true"]::before {
          background: #10b981; border-color: #10b981; color: #fff;
          content: "✓"; font-weight: 700; text-align: center; line-height: 16px;
        }
        ...
        .cxe[contenteditable="true"][data-placeholder]:empty::before {
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                disabled={busy}
                className="px-3 py-2 rounded-xl ring-1 ring-[#E5ECFF] bg-white text-sm inline-flex items-center gap-2 disabled:opacity-60"
              >
              ...
              <button
                ...
                disabled={busy}
                className="px-3 py-2 rounded-xl bg-[#1345DE] text-white text-sm inline-flex items-center gap-2 disabled:opacity-60"
              >
          ...
          <div className="rounded-2xl bg-white/95 ring-1 ring-[#E5ECFF] p-4">
            {sourceMode ? (
              <textarea
                ...
                rows={24}
                className="w-full rounded-xl ring-1 ring-[#E5ECFF] px-3 py-2 font-mono text-[13px] min-h-[520px]"
              />
            ) : (
                ...
                <ul className="mt-2 space-y-1 text-sm">
                  {outline.map(h => (
                    <li key={h.id} className="truncate">
                      <a href={`#${h.id}`} className={`hover:underline ${h.level===1?'font-semibold':'pl-3'}`}>• {h.text || '(без назви)'}</a>
                    </li>
                  ))}
                </ul>
...
function setFontFamily(_css: string) {}
function setFontSizePx(_px: string) {}
function setColor(_hex: string) {}
function setHighlight(_hex: string) {}
