...
export async function registerUser(
  ...
  email: string,
  password: string,
  birthDateISO: string, // формат YYYY-MM-DD
) {
  // Підлаштовуємо payload під типові бекенди (email як username, snake_case для дати)
  const payload = {
    role,
    name,
    email,
    password,
    username: email,         // якщо бек вимагає username — продублювали email
    birth_date: birthDateISO // бек зазвичай чекає snake_case
  };

  const { data } = await http.post(REGISTER_URL, payload);

  // Якщо бек повернув токени — збережемо (узгоджено з твоєю login-функцією)
  if (typeof window !== 'undefined') {
    if (data?.access) localStorage.setItem('access', data.access);
    if (data?.refresh) localStorage.setItem('refresh', data.refresh);
  }
  return data; // може містити access/refresh або просто профіль/ok
}
