...
/** ————— Іконки ————— */
const PhoneIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6 fill-[#1345DE]">
    <path d="M6.62 10.79a15.05 15.05 0 006.59 6.59l2.2-2.2a1 1 0 011.02-.24 11.36 11.36 0 003.56.57 1 1 0 011 1v3.59a1 1 0 01-1 1A17 17 0 013 5a1 1 0 011-1h3.59a1 1 0 011 1 11.36 11.36 0 00.57 3.56 1 1 0 01-.24 1.02l-2.3 2.21z" />
  </svg>
);
const MailIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6 fill-[#1345DE]">
...
const ClockIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6 fill-[#1345DE]">
    <path d="M12 2a10 10 0 1010 10A10 10 0 0012 2zm1 10.41l3.29 3.3-1.42 1.41L11 13V7h2z" />
  </svg>
);
const TelegramIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6 fill-[#0EA5E9]">
    <path d="M9.04 15.29 8.8 19.2c.43 0 .62-.18.84-.4l2.02-1.94 4.19 3.07c.77.43 1.31.2 1.52-.71l2.76-12.97c.24-1.12-.41-1.56-1.16-1.29L3.32 9.9c-1.09.43-1.07 1.06-.19 1.34л4.76 1.49 11.07-6.98c.52-.32 1-.14.61.2" />
  </svg>
);
const WhatsAppIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6 fill-[#16A34A]">
    <path d="M12.04 2a9.9 9.9 0 00-8.5 15l-1 3.7 3.8-1a9.9 9.9 0 004.7 1.2h.01A9.9 9.9 0 1012.04 2zm5.8 14.2c-.25.7-1.5 1.3-2.1 1.4-.55.1-1.25.2-4.3-1.4-3.6-1.9-5.9-5-6-5.2-.2-.2-1.4-1.9-1.4-3.7 0-1.9 1-2.8 1.4-3.2.4-.4.9-.5 1.2-.5h.9c.3 0 .7 0 1 .8.4 1 1.3 3.5 1.4 3.8.1.3.2.6 0 .9-.1.3-.2.5-.4.7-.2.2-.4.5-.6.7-.2.2-.4.5-.2.9.2.4 1 1.6 2.2 2.6 1.5 1.3 2.7 1.7 3.1 1.9.4.2.7.2 1-.1.3-.4 1.1-1.2 1.4-1.7.3-.5.6-.4 1-.2.4.2 2.5 1.2 2.9 1.4.4.2.7.3.8.5.1.2.1.7 0 1z" />
  </svg>
);
...
function ContactCard({
  ...
  return (
    <div className="rounded-[20px] bg-white ring-1 ring-[#E5ECFF] p-4 shadow-[0_8px_24px_rgba(2,28,78,0.06)] flex flex-col gap-3">
      ...
      <div className="flex flex-col">
        {items.map((it, idx) => (
          <a key={idx} href={it.href} className="inline-flex items-center gap-2 py-1 text-[#1345DE] hover:underline">
...
/** ————— Дані FAQ ————— */
const faqData = [
  { q: 'Як швидко ви відповідаєте?', a: 'У робочі години — зазвичай протягом 15–30 хвилин. У вихідні — у месенджерах упродовж дня.' },
  { q: 'Чи можу я повернути кошти за курс?', a: 'Так, у межах політики повернення. Напишіть у підтримку — підкажемо деталі саме по вашому кейсу.' },
...
export default function ContactsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({ name: '', email: '', topic: 'Питання щодо курсів', phone: '', message: '', agree: false });
      } else {
        const data = await res.json().catch(() => ({}));
        setStatus({ type: 'error', message: 'Помилка відправки: ' + (data?.error || res.status) });
      }
    } catch (err) {
  ...
  return (
    <main className="min-h-screen bg-[url('/images/back.png')] bg-cover bg-top">
      ...
      <section className="w-[1280px] mx-auto pt-[159px]">
        ...
        >
          ...
          <div className="w-[461px]">
            ...
            {/* Плашка — годи роботи */}
            <div className="mt-6 flex items-center gap-3 rounded-[14px] bg-white/80 ring-1 ring-[#E5ECFF] p-3 shadow-[0_6px_22px_rgba(2,28,78,0.08)]">
              <ClockIcon />
          ...
          <div
            ...
            style={{ gridTemplateColumns: '308px 239px', gridAutoRows: 'auto', columnGap: '17px', rowGap: '19px' }}
          >
            <ContactCard
              title="Email"
            ...
            <form onSubmit={handleSubmit} className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="md:col-span-2 flex flex-wrap items-center justify-between gap-4">
                <label className="inline-flex items-center gap-2 text-sm text-slate-600">
                  <input name="agree" type="checkbox" checked={formData.agree} onChange={handleChange} className="accent-[#1345DE]" />
        ...
        <div className="mx-auto max-w-[1160px] px-6 md:px-[118px] grid grid-cols-1 lg:grid-cols-[380px_minmax(0,1fr)] gap-6">
          <div className="rounded-[20px] bg-white ring-1 ring-[#E5ECFF] p-5 shadow-[0_8px_24px_rgba(2,28,78,0.06)]">
            ...
            <div className="mt-4 flex flex-col gap-2">
              <a className="text-[#1345DE] font-semibold hover:underline" href="https://maps.google.com/?q=Київ,Балтійська,23А" target="_blank">
                Відкрити в Картах Google
              </a>
              <a className="text-[#1345DE] hover:underline" href="https://maps.apple.com/?q=Київ,Балтійська,23А" target="_blank">
          ...
          <div className="mt-6 space-y-3">
            {faqData.map((f, i) => (
              <div key={i} className="rounded-[16px] bg-white ring-1 ring-[#E5ECFF] overflow-hidden">
                ...
                <div className={`px-4 pb-4 text-slate-700 ${openFaq === i ? 'block' : 'hidden'}`}>{f.a}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* CTA: Написати у месенджер / Соцмережі */}
      <section className="pb-16">
        <div className="mx-auto max-w-[1160px] px-6 md:px-[118px] grid grid-cols-1 md:grid-cols-2 gap-6">
          <a
            href="https://t.me/brainboost"
            className="rounded-[16px] bg-white ring-1 ring-[#E5ECFF] p-5 flex items-center gap-4 shadow-[0_8px_24px_rgba(2,28,78,0.06)] hover:ring-[#1345DE] transition"
          >
            <TelegramIcon />
            <div>
              <div className="text-[#0F2E64] font-extrabold">Telegram</div>
              <div className="text-slate-700 text-sm">Швидкі відповіді менеджера, статус замовлення, консультації</div>
            </div>
          </a>
          <a
          ...
          >
            ...
            <div>
              ...
              <div className="text-slate-700 text-sm">Питання щодо курсів та оплат, швидка допомога</div>
            </div>
          </a>
        </div>
      </section>

      {/* Невеликий блок-довіри */}
